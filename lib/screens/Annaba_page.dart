import 'package:cloud_firestore/cloud_firestore.dart';
import 'package:flutter/material.dart';
import 'package:hippone_museum/modules/Annaba.dart';
import 'package:hippone_museum/modules/Museum.dart';
import 'package:hippone_museum/services/be.dart';

class AnnabaPage extends StatefulWidget {
  const AnnabaPage({Key? key}) : super(key: key);
  static String id = "AnnabaPage";

  @override
  State<AnnabaPage> createState() => _AnnabaPageState();
}

class _AnnabaPageState extends State<AnnabaPage> {
  final _backend = BackEnd();
  FirebaseFirestore _firestore = FirebaseFirestore.instance;

  @override
  Widget build(BuildContext context) {
    return Scaffold(
      backgroundColor: Colors.black,
      extendBodyBehindAppBar: true,
      appBar: AppBar(
        backgroundColor: Colors.transparent,
        elevation: 0,
        foregroundColor: Colors.white,
      ),
      body: SingleChildScrollView(
        child: Column(
          children: [
            Image.asset(
              "assets/histoire de annba.jpg",
              fit: BoxFit.cover,
            ),
            const SizedBox(height: 20),
            const Divider(
              color: Colors.grey,
              thickness: 0.8,
            ),
            Padding(
              padding: const EdgeInsets.symmetric(horizontal: 20, vertical: 10),
              child: StreamBuilder<QuerySnapshot>(
                stream: _backend.loadAnnaba(),
                builder: (context, snapshot) {
                  if (snapshot.hasData) {
                    List<Annaba> _annaba = [];

                    for (var doc in snapshot.data!.docs) {
                      var data = doc;
                      _annaba.add(
                        Annaba(
                          imageUrl1:
                              data.data().toString().contains('ImageUrl1')
                                  ? doc.get('ImageUrl1')
                                  : "",
                          imageUrl2:
                              data.data().toString().contains('ImageUrl2')
                                  ? doc.get('ImageUrl2')
                                  : "",
                          text1: data.data().toString().contains('Text1')
                              ? doc.get('Text1')
                              : "",
                          text2: data.data().toString().contains('Text2')
                              ? doc.get('Text2')
                              : "",
                          text3: data.data().toString().contains('Text3')
                              ? doc.get('Text3')
                              : "",
                          text4: data.data().toString().contains('Text4')
                              ? doc.get('Text4')
                              : "",
                          text5: data.data().toString().contains('Text5')
                              ? doc.get('Text5')
                              : "",
                          title: data.data().toString().contains('Title')
                              ? doc.get('Title')
                              : "",
                        ),
                      );
                    }
                    return ListView.builder(
                      physics: NeverScrollableScrollPhysics(),
                      scrollDirection: Axis.vertical,
                      shrinkWrap: true,
                      itemBuilder: (context, index) => InkWell(
                          child: Column(
                        crossAxisAlignment: CrossAxisAlignment.center,
                        mainAxisAlignment: MainAxisAlignment.spaceEvenly,
                        children: [
                          Text(
                            _annaba[index].title,
                            textAlign: TextAlign.center,
                            style: TextStyle(
                                color: Colors.yellow.shade400, fontSize: 25),
                          ),
                          const SizedBox(
                            height: 20,
                          ),
                          Text(
                            _annaba[index].text1,
                            style: const TextStyle(
                                color: Colors.white, fontSize: 20),
                          ),
                          SizedBox(
                            height: 20,
                          ),
                          _annaba[index].imageUrl1.isNotEmpty
                              ? Image.network(
                                  _annaba[index].imageUrl1,
                                  fit: BoxFit.cover,
                                )
                              : Container(),
                          SizedBox(
                            height: 20,
                          ),
                          Text(
                            _annaba[index].text2,
                            style: const TextStyle(
                                color: Colors.white, fontSize: 20),
                          ),
                          SizedBox(
                            height: 20,
                          ),
                          Text(
                            _annaba[index].text3,
                            style: const TextStyle(
                                color: Colors.white, fontSize: 20),
                          ),
                          SizedBox(
                            height: 20,
                          ),
                          Text(
                            _annaba[index].text4,
                            style: const TextStyle(
                                color: Colors.white, fontSize: 20),
                          ),
                          SizedBox(
                            height: 20,
                          ),
                          Text(
                            _annaba[index].text5,
                            style: const TextStyle(
                                color: Colors.white, fontSize: 20),
                          ),
                          SizedBox(
                            height: 20,
                          ),
                          _annaba[index].imageUrl2.isNotEmpty
                              ? Image.network(
                                  _annaba[index].imageUrl2,
                                  fit: BoxFit.cover,
                                )
                              : Container(),
                          SizedBox(
                            height: 20,
                          ),
                        ],
                      )),
                      itemCount: _annaba.length,
                    );
                  } else {
                    return const Center(
                      child: Text("Loading..."),
                    );
                  }
                },
              ),
            ),
          ],
        ),
      ),
    );
  }
}
